...
(
    id          BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- id клиента
...
(
    id             BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- id банковского счёта
    account_number CHAR(20) UNIQUE,                                 -- Номер счёта
    balance        NUMERIC(12, 2),                                  -- Баланс
    status         VARCHAR(6),                                      -- Статус счёта
    client_id      BIGINT REFERENCES clients (id)                   -- id владельца счёта
);

CREATE TABLE actions
(
    id                  BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- id операции
    ...
    account_id          BIGINT NOT NULL REFERENCES accounts (id)         -- id счёта, на котором совершена операция
);